#!/usr/bin/env python3
"""
Top PR Count Repository Analyzer for AIDev Dataset

Analyzes pull request statistics for top N repositories by PR count in the AIDev dataset.
Generates comparative visualizations and reports across multiple repositories.

Unlike multi_repo_analyzer.py which selects by stars, this script selects repositories
with the highest number of AI-generated PRs (with optional minimum stars filter to
exclude experimental/test repositories).

Usage:
    uv run specific_repo_exploration/top_pr_repos_analyzer.py
    uv run specific_repo_exploration/top_pr_repos_analyzer.py --num-repos 15
    uv run specific_repo_exploration/top_pr_repos_analyzer.py -n 10 --min-stars 1000 -o ./output
"""
...
def get_top_n_repos_by_pr_count(
    repo_df: pd.DataFrame,
    pr_df: pd.DataFrame,
    n: int = 10,
    min_stars: int = 500
) -> pd.DataFrame:
    """Select top N repositories by PR count (with min stars filter).

    Args:
        repo_df: Repository dataframe
        pr_df: Pull request dataframe
        n: Number of top repositories to select
        min_stars: Minimum stars required to be considered (filters out experimental repos)

    Returns:
        DataFrame with top N repositories by PR count
    """
    star_col = 'stargazers_count' if 'stargazers_count' in repo_df.columns else 'stars'

    # Filter repos by minimum stars
    eligible_repos = repo_df[repo_df[star_col] >= min_stars].copy()
    print(f"Repositories with >= {min_stars} stars: {len(eligible_repos)}")

    # Count closed PRs per repo
    closed_prs = pr_df[pr_df['state'] == 'closed']
    pr_counts = closed_prs.groupby('repo_id').size().reset_index(name='pr_count')

    # Filter to eligible repos only
    pr_counts = pr_counts[pr_counts['repo_id'].isin(eligible_repos['id'])]

    # Merge with repo info
    pr_counts = pr_counts.merge(eligible_repos, left_on='repo_id', right_on='id', how='left')

    # Sort by PR count and get top N
    top_repos = pr_counts.nlargest(n, 'pr_count')

    print(f"\nSelected top {len(top_repos)} repositories by PR count (min {min_stars} stars):")
    for _, row in top_repos.iterrows():
        print(f"  - {row['full_name']}: {row['pr_count']} PRs, {row[star_col]:,} stars")
...
def create_pr_status_bar_chart(status_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set_title('PR Status by Repository (Top by PR Count)', fontsize=13, fontweight='bold', pad=15)
    ax.set_xticks(x)
...
def generate_markdown_report(
    ...
    report = []
    report.append(f"# Top PR Count Repository Analysis Report")
    report.append("")
    report.append(f"**Generated:** {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append("")
    report.append(f"**Configuration:**")
    report.append(f"- Number of repositories analyzed: {args.num_repos}")
    report.append(f"- Minimum stars required: {args.min_stars}")
    ...
    # Top Repositories
    report.append("## Top Repositories by PR Count")
    report.append("")
    report.append("| # | Repository | PRs | Stars | Language | License |")
    report.append("|---|------------|-----|-------|----------|---------|")
    for idx, (_, repo) in enumerate(top_repos.iterrows(), 1):
        report.append(f"| {idx} | {repo['full_name']} | {repo['pr_count']} | {repo[star_col]:,} | {repo.get('language', 'N/A')} | {repo.get('license', 'N/A')} |")
    report.append("")
    ...
    report.append("")
    report.append("```bash")
    report.append("# Default: analyze top 10 repositories by PR count (min 500 stars)")
    report.append("uv run specific_repo_exploration/top_pr_repos_analyzer.py")
    report.append("")
    report.append("# Analyze top N repositories")
    report.append("uv run specific_repo_exploration/top_pr_repos_analyzer.py --num-repos 15")
    report.append("")
    report.append("# Custom configuration")
    report.append("uv run specific_repo_exploration/top_pr_repos_analyzer.py -n 10 --min-stars 1000 --min-turnaround 120 --top-n-percent 5")
    report.append("```")
    report.append("")
...
def analyze_top_pr_repos(args) -> None:
    ...
    print("=" * 60)
    print("Top PR Count Repository Analyzer for AIDev Dataset")
    print("=" * 60)
    print()
    ...
    top_repos = get_top_n_repos_by_pr_count(
        repo_df, pr_df,
        n=args.num_repos,
    ...
    star_col = 'stargazers_count' if 'stargazers_count' in top_repos.columns else 'stars'
    top_repos[['full_name', 'pr_count', star_col, 'language', 'license']].to_csv(
    ...
    summary = {
        ...
        'min_stars_filter': args.min_stars,
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-repository PR analysis for top N repos by PR count in AIDev dataset"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Minimum stars for repository to be considered (default: 500)"
    )

    parser.add_argument(
        "--output", "-o",
        type=str,
        default="./specific_repo_exploration/top_pr_repos_analysis",
